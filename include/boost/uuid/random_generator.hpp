...
private:
    ...
    seed(MaybePseudoRandomNumberGenerator& rng)
    {
        ...
        using detail::random::random_device;

        boost::infinite inf;
        random_device egen;
        function_input_iterator<random_device, boost::infinite> infit = make_function_input_iterator(egen, inf);
        rng.seed(infit, infit);
    }
...
// does.
//
typedef basic_random_generator<detail::random::random_device> random_generator;

//
// The bulk generator uses a mersenne twister to make random uuid generation of
// many uuids from a single generator more efficient, at the expense of security
