...
#if defined(BOOST_UUID_RANDOM_PROVIDER_BCRYPT)
...
#elif defined(BOOST_UUID_RANDOM_PROVIDER_CLOUDABI) 

#include <stdexcept>

//
// No idea how to do sad path testing on CloudABI yet
//
...
void expect_next_call_success(bool success)
{
    // If there is nothing in the list and success is false, we want both
    // calls to mockopen() to fail - once for /dev/urandom and once for /dev/random
    if (posix_next_result.empty() && !success)
    {
        // Add an extra one
